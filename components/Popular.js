    ...
    return (
        <section className="bg-black text-white">
            ...
            <div className="flex w-[90vw] text-sm md:text-md md:w-[80vw] mx-auto flex-col md:flex-row m-4 gap-2">
                <p className="mx-2 font-semibold text-white text-2xl">What&apos;s Popular</p>
                <div className="flex gap-4 overflow-none mt-3 md:mt-0">
                    {options.map(({ name, state, setter, fetchFunction }) => (
                        <div
                            key={name}
                            className={`py-2 px-4 rounded-full cursor-pointer flex items-center transition-all duration-300  ${state ? "bg-red-600 text-white scale-105" : "bg-gray-700 text-gray-300 hover:bg-red-500 hover:text-white"} text-sm md:text-xs`}
                            onClick={() => {
                                if (!state) {
