    ...
    return (
        <section>
            <div className="title text-sm md:text-md w-[90vw] md:w-[80vw] mx-auto flex md:items-center gap-3 md:gap-10 md:mt-8 md:flex-row flex-col">
                <p className="mx-2 font-semibold text-black text-2xl">Free To Watch</p>
                <div className="flex gap-4  w-fit">
                    <div
                        className={`py-2 px-4 rounded-full cursor-pointer flex items-center transition-all duration-300 ${isMovie ? "bg-red-600 text-white scale-105" : "bg-gray-200 text-gray-700 hover:bg-red-500 hover:text-white"} 
        text-sm md:text-xs`}
        ...
        ${!isMovie ? "bg-red-600 text-white scale-105" : "bg-gray-200 text-gray-700 hover:bg-red-500 hover:text-white"} 
        text-sm md:text-xs`}
            ...
            <div className="flex w-[90vw] md:w-[80vw] mx-auto overflow-x-auto m-4 gap-2">
                {loading ? (
                    ...
                    <div>No {isMovie ? 'movies' : 'shows'} available</div>
                )}
            </div>
        </section>
    );
